...
// Route tạo thanh toán (dùng Strategy)
router.post(
  "/create",
  authMiddleware(["tenant"]),
  rentController.createRentPayment
);

// Route xử lý callback
router.get(
  "/execute",
  authMiddleware(["tenant"]),
  rentController.executeRentPayment
);
...
router.post(
  "/vnpay/create",
  authMiddleware(["tenant"]),
  rentController.createVNPayPayment
);
...
router.get("/payment-result", authMiddleware(["tenant"]), (req, res) => {
  ...
  if (vnp_ResponseCode === "00") {
    res.redirect(
      `${process.env.CLIENT_URL}/tenant/bills/payment-result?vnp_ResponseCode=${vnp_ResponseCode}&vnp_TxnRef=${vnp_TxnRef}`
    );
  } else {
    res.redirect(
